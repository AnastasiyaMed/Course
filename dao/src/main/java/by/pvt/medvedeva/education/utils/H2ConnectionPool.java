...
//    }
    ...
    private H2ConnectionPool() throws SQLException {
        try (Connection c = getConnect()) {
            ...
            c.createStatement().execute(H2DbSQLManager.getProperty("REPLACE_INTO_COURSE_TABLE_SQL"));
            c.createStatement().execute(H2DbSQLManager.getProperty("CREATE_STUDENT_TABLE_SQL"));
            c.createStatement().executeUpdate(H2DbSQLManager.getProperty("REPLACE_INTO_STUDENT_TABLE_SQL"));
            ...
            c.createStatement().execute(H2DbSQLManager.getProperty("REPLACE_INTO_TEACHER_TABLE_SQL"));
            c.createStatement().execute(H2DbSQLManager.getProperty("CREATE_USER_TABLE_SQL"));
            c.createStatement().execute(H2DbSQLManager.getProperty("REPLACE_INTO_USER_TABLE_SQL"));
    }
    }
